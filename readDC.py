...
def runLimitCalc (filename) :
    ...
    calcLimit = subprocess.check_output (args)
    localoutfile = filename.replace ('tempo', 'result')  
    f = open (localoutfile, 'w')
    f.write (calcLimit)
    f.close ()
...
#    gStyle.SetGridWidth (float (0.5))
    ...
    nominalLimit = getLimit (datacardname)
